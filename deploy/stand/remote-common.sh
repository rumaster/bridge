...
# Разворачивает "a,b,c" в строки по одной на элемент (пустая строка → ничего).
#
# Завершающий '\n' обязателен: `while read` возвращает false на EOF без перевода
# строки, и последний элемент молча терялся бы — при одном элементе список
# оказывался пуст целиком. (`mapfile` незавершённую строку забирает, поэтому
# баг был виден не везде.)
csv_to_args() {
  local csv=${1:-}
  if [ -z "$csv" ]; then
    return 0
...
upsert_env() {
  ...
  # Явный if, а не `[ ... ] && ...`: под `set -e` AND-список с ложным условием
  # возвращает 1 и роняет скрипт — ровно так же ломался authorized_keys стенда.
