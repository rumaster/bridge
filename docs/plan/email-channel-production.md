...
1. **Retention/GC байтов на RF-томе — СДЕЛАНО (MVP по TTL).** Ранее файлы в
   `EMAIL_ATTACHMENT_STORAGE_DIR` (`bridge-edge-attachments`) **не удалялись
   никогда** — том рос безгранично. Реализована политика очистки по TTL от
   времени записи объекта (mtime): сборщик на Edge
   ([`edge-attachment-gc.ts`](../../services/edge-gateway/src/edge-attachment-gc.ts))
   периодически вызывает `EdgeAttachmentStore.sweep`
   ([`edge-attachment-store.ts`](../../services/edge-gateway/src/edge-attachment-store.ts)),
   удаляя просроченные объекты (байты + sidecar) с метриками удалённого/освобождённого.
   Конфиг: `EMAIL_ATTACHMENT_TTL_DAYS` (0/пусто → GC выключен, прежнее поведение)
   и `EMAIL_ATTACHMENT_GC_INTERVAL_MS` (дефолт 6 ч). Дедуп по content-hash учтён:
   удалённый объект детерминированно пересоздаётся при следующем письме с теми же
   байтами. Точечная защита «живых» объектов от TTL — через хук `sweep({ isLive })`
   (точка расширения). **Остаётся (не MVP):** GC-по-ссылкам — App-сторона отдаёт
   множество живых `storage_ref` из таблицы `attachments` через control-plane, и
   сборщик не трогает объект, на который ссылается хотя бы одно сообщение (плюс
   немедленная очистка по удалению сообщения/диалога вместо ожидания TTL).
