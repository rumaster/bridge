...
### M3 — Deliverability и боевая доставка в интернет — 🟡 ПОДГОТОВЛЕНО / боевая доставка отложена

Превращает мишень в реальный MTA. **Это основная сложность продукта.**

> **Статус: подготовлено, боевая доставка сознательно отложена** (решение по
> итогам разведки стенда). Что сделано:
...
> - **DNS-записи подготовлены** (MX/A/SPF/DKIM/DMARC + заявка на PTR) —
>   [`deploy/mail/dns-records.md`](../../deploy/mail/dns-records.md), готовы к
>   публикации (владелец DNS домена их добавляет).
> - **Разведка выявила жёсткие блокеры боевой доставки на этом стенде**, которые
>   вне контроля кода:
>   1. исходящий **порт 25 заблокирован** → прямая доставка в интернет невозможна
>      (нужен relay через транзакционный SMTP 587/465 либо разблокировка
>      провайдером — риск §7.1);
>   2. **PTR = `bridge.`** (не FQDN) → чинится только у провайдера IP;
>   3. домен `lissac-games.online` указывает на другой хост (`13.60.43.104`),
>      MX/SPF/DMARC отсутствуют → записи добавляет владелец DNS.
> - Продовые тумблеры (`ENABLE_RSPAMD/OPENDKIM/OPENDMARC/FAIL2BAN`, `SSL_TYPE`)
>   сознательно НЕ включены на стенде, чтобы не ломать рабочий M1/M2-контур;
>   значения для включения задокументированы в `dns-records.md` §«Что ещё нужно».
>
> **Оставшееся для боевой доставки — вне текущего окружения:** relay/разблокировка
> 25, корректный PTR, публикация DNS, TLS-сертификат, публикация портов
> `25/465/587/993`. Возобновляется на площадке со статическим IP, открытым 25
> (или relay) и управляемым PTR.

- Публикация портов 25/465/587/993 и сетевой доступ; статический IP c **PTR**
  (reverse DNS) = `MAIL_HOSTNAME`.
- DNS-записи домена: **MX** → `MAIL_HOSTNAME`; **A/AAAA**; **SPF** (`v=spf1 mx
  -all`); **DKIM** (сгенерировать `setup config dkim`, ключ — в
  `deploy/mail/opendkim`, публичная часть — в DNS TXT); **DMARC**
  (`v=DMARC1; p=quarantine; rua=...`).
- Включить `ENABLE_RSPAMD=1`, `ENABLE_OPENDKIM=1`, `ENABLE_FAIL2BAN=1`,
  `SSL_TYPE=letsencrypt` (или проброс своих TLS-сертификатов).
- Прогрев IP/репутации, проверка на mail-tester.com / Google Postmaster Tools,
  контроль попадания в spam-листы.
...
### M5 — Продуктивизация «Bridge Mail» как услуги (по потребности)
...
1. **Deliverability — главный риск продукта.** Свой MTA без прогретого IP и
   ...
   провайдеров. Реалистичная альтернатива на M3 — **relay исходящей почты через
   транзакционный SMTP** (свой Postfix принимает от клиента, но пересылает через
   внешний relay с хорошей репутацией). Решить на входе в M3: полный
   self-hosted MTA vs. self-hosted приём + relay-отправка.
