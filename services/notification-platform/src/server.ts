...
import { C10DtoValidationError } from "./c10-dto.js";
import {
  C10NotificationNotFoundError,
  createDeterministicNotificationMock,
} from "./deterministic-notification.js";
...
function renderMetrics(metrics) {
  const lines = [
    ...
    "# HELP notification_platform_mock_delivery_failed_total Channel deliveries that failed after all retry attempts.",
    "# TYPE notification_platform_mock_delivery_failed_total counter",
    `notification_platform_mock_delivery_failed_total{channel="web"} ${metrics.delivery_web_failed_total}`,
    `notification_platform_mock_delivery_failed_total{channel="telegram"} ${metrics.delivery_telegram_failed_total}`,
    `notification_platform_mock_delivery_failed_total{channel="email"} ${metrics.delivery_email_failed_total}`,
    `notification_platform_mock_delivery_failed_total{channel="push"} ${metrics.delivery_push_failed_total}`,
    "# HELP notification_platform_mock_delivery_retry_total Additional delivery attempts after an initial channel failure.",
    "# TYPE notification_platform_mock_delivery_retry_total counter",
    `notification_platform_mock_delivery_retry_total ${metrics.delivery_retry_total}`,
