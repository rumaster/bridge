...
describe("Внутренний messaging-путь (issue #189/#191)", () => {
  ...
  it("прокидывает исходящие вложения (storage_ref) в конверт egress (§4.3-bis follow-up п.2)", async () => {
    ...
    // Сохранённое исходящее сообщение + строка вложения (её кладёт createMessage
    // при отправке менеджером; здесь фиксируем фикстурой напрямую в БД).
    await withClient(databaseUrl, async (client) => {
      await setPlatformOperator(client);
      await client.query(
        `
          ...
          VALUES ($1, $2, $3, $4, 'telegram', 'outbound', 'manager', 7, 'text',
            '{"text":"файл во вложении"}'::jsonb, 'routed', '2026-07-04T08:45:00.000Z')
        `,
        [attachOutMessage, ORG, OUT_CONVERSATION, OUT_ENDPOINT],
      );
      await client.query(
        `
