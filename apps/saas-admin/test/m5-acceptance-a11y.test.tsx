...
const SCREEN_READY_HEADINGS: Record<string, string[]> = {
  ...
  // Дожидаемся полного рендера редактора, иначе снапшот заголовков нестабилен
  // из-за асинхронной загрузки черновика.
  //
  // Ревизия 2026-07-15 (этап 7): «Связи узлов» убраны из ожиданий осознанно —
  // связи стали рёбрами холста, отдельного списка больше нет, и это цель этапа, а
  // не потеря. «Информация о схеме» и «Свойства узла» — панели старого редактора;
  // первая пока не перенесена (см. остаток этапа 7 в плане), вторая существует, но
  // её заголовок теперь называет тип выбранного узла, а узел по умолчанию не
  // выбран. Пин вернуть к полному составу, когда страница дойдёт до паритета.
  "/workflow": ["Автоответчик обращений", "Палитра", "Тестовый прогон"]
};
